...
class Actor(nn.Module):
    def __init__(self, state_dim, hidden_dim=64, output_size=2):
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=64, output_size=1):
...
class DDPG(object):
    def __init__(self, state_dim, action_dim, action_bound, tau=0.01,
                 gamma=0.99, init_epsilon=1.0, final_epsilon=0.01, entropy_weight=0.01,
                 batch_size=128, pool_size=10000, actor_lr=0.0001, critic_lr=0.0001):
        ...
        # four networks
        self.target_actor = Actor(state_dim)  # type: Actor
        self.current_actor = Actor(state_dim)  # type: Actor
        self.target_critic = Critic(state_dim, action_dim)  # type: Critic
        self.current_critic = Critic(state_dim, action_dim)  # type: Critic
    ...
    def ou(a_v, mu=0, theta=0.05, sigma=0.025):
        ...
        :param a_v: 动作
        :param mu: 均值
        :param theta: 离均值的改变幅度 0.05 (太大0.5效果不好)
        :param sigma: 随机噪声的权重 0.025 (太大0.25效果不好)
        :return: OU value
        """
        return theta * (mu - a_v) + sigma * np.random.randn(1)

    def infer_action(self, state, step=1, greedy=False):
        # 动作为连续动作，使用clip可以划定范围
        state_embedding = torch.tensor(state, dtype=torch.float)
        action_ = self.current_actor(state_embedding).detach().sum(0).item()  # current_actor net bias很大
        # print('state', state, '\t', 'action', action_)
        # print('actor_parameters', self.current_actor.state_dict())
        ...
        if not greedy:
            ...
            action = np.clip(action_ + noise, -self.action_bound + 0.01, self.action_bound - 0.01)
            return action
        else:
            return action_
    ...
    def learn(self):
        ...
        td_target = reward_batch + self.gamma * self.target_critic(next_state_batch,
                                                                   next_actions).detach()
        cur_critic_loss = f.smooth_l1_loss(input=self.current_critic(state_batch, action_batch), target=td_target)
...
if __name__ == '__main__':
    ...
    for episode in range(episode_size):
        ...
        for step_ in range(step_size):
            ...
            if d:
                print('The episode %d is over after %d steps | reward: %0.2f' % (episode, step_, total_reward))
                break

            obs = n_s
        total_reward_list.append(total_reward)

        if episode >= 200 and np.mean(total_reward_list[-100:]) >= save_theshold:
            print('Successfully')
            torch.save(model, '../model/ddpg_model.th')
            break
